...
from vaultdiskfsstorage, ( 
    select sum(c) as size, id_fs from (
                (
                SELECT CASE WHEN sum(vaultdiskstorage.size) IS NULL THEN 0 ELSE sum(vaultdiskstorage.size) END AS c,
                vaultdiskdirstorage.id_fs
                FROM vaultdiskstorage RIGHT JOIN vaultdiskdirstorage
                ON (vaultdiskdirstorage.id_fs = vaultdiskstorage.id_fs AND vaultdiskdirstorage.id_dir = vaultdiskstorage.id_dir)
                where (not isfull or isfull is null)
