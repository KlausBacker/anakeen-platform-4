...
create sequence seq_id_users start 10;";
    ...
    public function getSubstitute()
    {
        if ($this->isAffected()) {
            if ($this->substitute) {
                return new \Anakeen\Core\Account($this->dbaccess, $this->substitute);
            }
        }
        return null;
    }
    ...
    public function preInsert()
    {
        ...
        if (($this->accounttype == self::GROUP_TYPE) || ($this->accounttype == self::ROLE_TYPE)
            || ($this->isgroup == "Y")) {
            if ((!$this->accounttype) && ($this->isgroup == "Y")) {
    ...
    public function PostInsert()
    {
        ...
        $gid = \Anakeen\Core\Account::GALL_ID; //2 = default group
        $group->idgroup = $gid;
...
union
 select iduser, users.login, users.mail from groups,users, amembers where groups.idgroup = amembers.uid and users.id=groups.iduser
    ...
    public function PostInit()
    {
        ...
        $userAdmin = new \Anakeen\Core\Account($this->dbaccess);
        // Create admin user
        $userAdmin->id = \Anakeen\Core\Account::ADMIN_ID;
        ...
        $groupAll = new \Anakeen\Core\Account($this->dbaccess);
        $groupAll->id = \Anakeen\Core\Account::GALL_ID;
        ...
        $anonymousUser = new \Anakeen\Core\Account($this->dbaccess);
        $anonymousUser->id = \Anakeen\Core\Account::ANONYMOUS_ID;
        ...
        $groupAdmin = new \Anakeen\Core\Account($this->dbaccess);
        $groupAdmin->id = \Anakeen\Core\Account::GADMIN_ID;
        $groupAdmin->lastname = "Administrateurs";
        $groupAdmin->firstname = "";
        $groupAdmin->login = "gadmin";
        $groupAdmin->accounttype = self::GROUP_TYPE;
        $groupAdmin->Add(true);
        $group->idgroup = \Anakeen\Core\Account::GALL_ID;
        $group->iduser = \Anakeen\Core\Account::GADMIN_ID;
        $group->Add(true);
        ...
        if ($incumbents = $this->getIncumbents()) {
            if ($doc->control($acl, true) != '') {
                foreach ($incumbents as $aIncumbent) {
                    ...
                    if (!$eErr) {
                        return \Anakeen\Core\Account::getDisplayName($aIncumbent);
                    }
                }
            }
        }
...
union
 ...
 select idgroup, users.login, users.accounttype from groups,users, agroups where groups.iduser = agroups.gid and users.id=groups.idgroup
    ...
    public function isMember($uid)
    {
        ...
        $sql = sprintf(
            "select users.id from users, groups where %s and (groups.iduser=users.id) and users.id=%d and isgroup != 'Y'",
            $cond,
            $uid
        );
    ...
    public function setRoles(array $roleIds)
    {
        ...
        if ($this->accounttype == self::GROUP_TYPE) {
            ...
            $uw = new \Anakeen\Core\Account($this->dbaccess);
            foreach ($lu as $u) {
                $uw->id = $u["id"];
                $uw->updateMemberOf();
            }
