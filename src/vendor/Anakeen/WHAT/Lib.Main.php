...
function handleActionException($e)
{
    ...
    $displayMsg = \Anakeen\Core\Anakeen\Core\Internal\LogException::logMessage($e, $errId);
 ...
 * @param Throwable $e
 *
 * @deprecated use Anakeen\Core\Anakeen\Core\Internal\LogException::formatErrorLogException()
 * @return string
 */
function formatErrorLogException($e)
{
    return \Anakeen\Core\Anakeen\Core\Internal\LogException::formatErrorLogException($e);
}
...
{
    \Anakeen\Core\Anakeen\Core\Internal\LogException::writeLog($e);
}
...
function handleFatalShutdown()
{
    ...
    if ($error !== null && $action) {
        if (in_array($error["type"], array(
            ...
            $displayMsg = \Anakeen\Core\Anakeen\Core\Internal\LogException::logMessage($error, $errId);
            if ($action) {
