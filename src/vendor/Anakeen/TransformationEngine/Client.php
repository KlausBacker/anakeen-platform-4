...
class Client
{
     ...
     */
    public function sendTransformation($te_name, $fkey, $filename, $callback, &$info)
    {
        $err = "";
        ...
        */
        ...
        $mime = FileMime::getSysMimeFile($filename, $basename);
     ...
     * @return string error message, if no error empty string
     */
    public function getInfo($tid, &$info)
    {
        $err = "";
        /* Lit l'adresse IP du serveur de destination */
        $address = gethostbyname($this->host);
        ...
        //    echo "Essai de connexion à '$address' sur le port '$service_port'...\n";
        //    $result = socket_connect($socket, $address, $service_port);
        $timeout = floatval(ContextParameterManager::getValue(Manager::Ns, "TE_TIMEOUT", 3));
     ...
     * @return string error message, if no error empty string
     */
    public function getTransformation($tid, $filename)
    {
     ...
     * @return string error message, if no error empty string
     */
    public function getAndLeaveTransformation($tid, $filename)
    {
        $err = "";
     ...
     * @return string error message, if no error empty string
     */
    public function abortTransformation($tid)
    {
        $err = "";
        /* Lit l'adresse IP du serveur de destination */
        $address = gethostbyname($this->host);
    ...
    {
        try {
            $this->genericCommandWithJSONResponseEx($cmd, $responseData);
        } catch (ClientException $e) {
            return $e->getMessage();
        }
        return '';
    ...
    private function genericCommandWithJSONResponseEx($cmd, &$responseData)
    {
        try {
            $sock = $this->connect();
        } catch (ClientException $e) {
    ...
    public function retrieveServerInfo(&$serverInfo, $extended = false)
    {
        try {
            if ($extended) {
                $this->genericCommandWithJSONResponseEx("INFO:SERVER:EXTENDED\n", $serverInfo);
            } else {
                $this->genericCommandWithJSONResponseEx("INFO:SERVER\n", $serverInfo);
            }
