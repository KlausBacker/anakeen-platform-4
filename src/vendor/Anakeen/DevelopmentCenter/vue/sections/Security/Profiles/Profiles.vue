<template>
    <div class="security-se-section">
        <ank-splitter class="profile-splitter" ref="profileSplitter" :panes="panes" localStorageKey="security-profiles-splitter">
            <template slot="left">
                <ank-se-grid ref="profilesGrid" :pageSizes="[100, 200, 500]" :contextTitles="false" urlConfig="/api/v2/devel/security/profiles/gridConfig" filterable="inline" @before-grid-cell-render="cellRender" @action-click="actionClick">
                </ank-se-grid>
            </template>
            <template slot="right">
