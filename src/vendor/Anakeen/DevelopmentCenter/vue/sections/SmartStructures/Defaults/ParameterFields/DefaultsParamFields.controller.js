...
export default {
  ...
  mounted() {
    this.remoteDataSource = new kendo.data.TreeListDataSource({
      ...
      schema: {
        data: response => {
          return this.getDefaultParamValues(response.data.data);
        },
        model: {
          id: "id",
          parentId: "parentId",
    ...
    $(window).resize(() => {
      if (this.$refs.defaultsTree) {
        this.$refs.defaultsTree.kendoWidget().resize();
      }
    });
  },
  methods: {
    ...
    refreshDefaults() {
      this.$refs.defaultsTree.kendoWidget().dataSource.filter({});
      this.$refs.defaultsTree.kendoWidget().dataSource.read();
    },
    ...
    getDefaultParamValues(response) {
