...
export default class VaultInfoController extends Vue {
  ...
  public logicalTemplate(data) {
    return `<b>${this.convertBytes(this.info.metrics.usedSize)}</b> used on <b>${this.convertBytes(this.info.metrics.totalSize)}</b> of logical capacity (<b>${Math.floor((this.info.metrics.usedSize / this.info.metrics.totalSize) * 100)}%</b>)`;
  }
  public diskTemplate(){
    return `<b>${this.convertBytes(this.info.disk.usedSize)}</b> used on <b>${this.convertBytes(this.info.disk.totalSize)}</b> of disk capacity (<b>${Math.floor((this.info.disk.usedSize / this.info.disk.totalSize) * 100)}%</b>)`;
  }
  public mounted() {
