...
class FileMime
{
    ...
    public static function getIconMimeArray()
    {
        return array(
    ...
    /**
     * Get basename of iconf for a sys mime
     *
     * @param string $sysmime file system mime
     *
     * @return string icon filename
     */
    public static function getIconMimeFile($sysmime)
    {
        $icon = "";
        if ($sysmime) {
     ...
     */
    public static function getSysMimeFile($f, $fn = "")
    {
        if (!file_exists($f)) {
            return false;
        }

        $ret = self::getMimeFile($fn, 'sys');
        if ($ret !== false) {
            return $ret;
        }

        $sys = trim(shell_exec(sprintf("file --mime -b %s", escapeshellarg($f))));
        $txt = self::getTextMimeFile($f);
        $sys = strtok($sys, " ;\n\t"); // only first part
    ...
    /**
     * Get textual mime type of a file
     *
     * @param   string $f  file path
     * @param string   $fn filename if search from name first
     *
     * @return bool|string
     */
    public static function getTextMimeFile($f, $fn = '')
    {
        $ret = self::getMimeFile($fn, 'text');
        if ($ret !== false) {
            return $ret;
        }
     ...
     */
    public static function getExtension($smime)
    {
     ...
     * @return bool
     */
    public static function getMimeFile($filename, $type = 'sys')
    {
        $conf_user = self::loadUserMimeConf();
        $conf_global = self::loadMimeConf();
