...
class DocFormFormat
{
     ...
     * @return mixed|string
     */
    public function getHtmlInput(&$oattr, $value, $index = "")
    {
        global $action;
        $this->oattr = $oattr;
        $this->index = $index;
        ...
        if ($this->index !== "") {
            $attridk = $this->oattr->id . '_' . $this->index;
        } else {
            $attridk = $this->oattr->id . $this->index;
        }
        ...
        if (isset($this->oattr->mvisibility)) {
            $this->visibility = $this->oattr->mvisibility;
        } else {
            $this->visibility = $this->oattr->visibility;
        }
        ...
        if (($this->visibility == "H") || ($this->visibility == "R")) {
            ...
            $input.= " > ";
            if (!$this->notd) {
                $input.= '</td><td class="hiddenAttribute">';
            }
            return $input;
        }
                ...
                if (($this->oattr->repeat) && (!$this->oattr->inArray())) { // textlist
                    ...
                    if ($this->oattr->eformat != "") {
                        ...
                        if ($this->getLayTextOptions($lay, $this->doc, $this->oattr, $value, $attrin, $this->index)) {
                            ...
                            if ($this->oattr->eformat == "hlist") {
                                $lay->set("atype", "hidden");
                            } else {
                                $lay->set("atype", "text");
                            }
                            $input = $lay->gen();
        ...
        if (($this->oattr->type != "array")) {
            if ($this->visibility != "S") {
                if ($usephpfunc && ($this->oattr->phpfunc != "") && ($this->oattr->phpfile != "") && ($this->oattr->type != "enum") && ($this->oattr->type != "enumlist")) {
                    ...
                    if ($this->oattr->getOption("ititle") != "") {
                        $ititle = str_replace("\"", "&quot;", $this->oattr->getOption("ititle"));
                    }
                    if ($phpfunc[0] == "[") {
                ...
                } elseif (($this->oattr->type == "date") || ($this->oattr->type == "timestamp")) {
            ...
            if ($this->oattr->elink != "" && (!$alone)) {
                ...
                if ($this->oattr->getOption("elsymbol") != "") {
                    $isymbol = $this->oattr->getOption("elsymbol");
                }
                if ($this->oattr->getOption("eltitle") != "") {
                    $ititle = str_replace("\"", "'", $this->oattr->getOption("eltitle"));
                }
    ...
    private function formatArray()
    {
        ...
        if ($rn) {
            $this->getZoneLayArray($lay, $this->doc, $this->oattr, $rn);
        } else {
            $this->getLayArray($lay, $this->doc, $this->oattr);
        }
    ...
    private function formatDocid($value)
    {
        ...
        if ($famid) {
            ...
            if ($multi == "yes") {
                ...
                if ($this->doc->usefor == "D") {
                    $input = "<input type=\"text\" title=\"" . _("real value to set") . "\" name=\"" . $this->attrin . "\"";
                } else {
                    $input = "<input type=\"hidden\"  name=\"" . $this->attrin . "\"";
                }
    ...
    private function formatEnum($value)
    {
        ...
        $trueNotd = $this->notd;
        if ($this->oattr->eformat == "") {
            $this->oattr->eformat = $this->oattr->getOption("eformat");
        }
        if (($this->oattr->repeat) && (!$this->oattr->inArray())) { // enumlist
            ...
            if ($value == "") {
                if (($this->oattr->eformat == 'bool') || ($this->oattr->getOption("eunset") != "yes")) {
                    $value = key($enuml);
                } else {
                    $value = " ";
                }
            ...
            switch ($this->oattr->eformat) {
                    ...
                    case "bool":
                        ...
                        if ($value == key($enuml)) {
                            $lay->set("checkedyesno", "checked");
                        } else {
                            $lay->rSet("checkedyesno", "");
                        }
        ...
        if (($this->visibility == "R") || ($this->visibility == "S")) {
            $lay->rSet("disabled", $this->idisabled);
        } else {
            $lay->rSet("disabled", "");
        }
    ...
    private function formatDate($value)
    {
        ...
        if (($this->visibility == "R") || ($this->visibility == "S")) {
            ...
            $lay->set("disabled", ' readonly="readonly"');
        }
    ...
    public function getLayArray(&$lay, &$doc, &$oattr, $row = - 1)
    {
        ...
        */
        ...
        if (($oattr->getOption("vlabel") == "") || ($oattr->getOption("vlabel") == "up")) {
    ...
    private function getZoneLayArray(&$lay, &$doc, &$oattr, $zone)
    {
        ...
        if (($zone != "") && preg_match("/([A-Z_-]+):([^:]+):{0,1}[A-Z]{0,1}/", $zone, $reg)) {
            ...
            if ($oattr->getOption("vlabel", "up") == "up") {
                $lay->set("caption", $oattr->getLabel());
            } else {
                $lay->set("caption", "");
            }
    ...
    public static function rowattrReplace(&$doc, $s, $index, &$defval = null)
    {
        ...
        if (substr($s, 0, 2) == "V_") {
            $s = substr($s, 2);
            if ($index != - 1) {
                $value = $doc->getMultipleRawValues($s, "", $index);
            } else {
                $value = $defval[strtolower($s) ];
            }
            $oattr = $doc->getAttribute($s);
            if (!$oattr) {
                return sprintf(_("unknow attribute %s"), $s);
            }
            ...
            if ($index == - 1) {
                $v = $doc->getRawValue($sl);
            } else {
                $v = $doc->getMultipleRawValues($sl, "", $index);
            }
            $v = str_replace('"', '&quot;', $v);
        }
        return $v;
    ...
    private function getLayDocOption(&$lay, &$doc, &$oattr, $value, $aname, $index)
    {
        ...
        if ($index !== "") {
            $lay->set("said", $doc->getMultipleRawValues($oattr->format, "", $index));
        } else {
            $lay->set("said", $doc->getRawValue($oattr->format));
        }
    ...
    private function getLayTextOptions(&$lay, &$doc, &$oattr, $value, $aname, $index)
    {
        ...
        if ($res === false) {
            return false;
        } // one or more attribute are not set
        $sattrid = "[";
        if (is_array($rargids)) {
     ...
     */
    public function sortEnumMap($enumMap, $sortBy)
    {
        global $action;
