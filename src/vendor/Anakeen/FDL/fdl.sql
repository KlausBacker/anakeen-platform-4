...
if ((TG_OP = ''UPDATE'') OR (TG_OP = ''INSERT'')) then
  if  NEW.doctype != ''T'' then
	...
	insert into docread(id,owner,title,revision,initid,fromid,doctype,locked,allocated,icon,lmodify,profid,views,usefor,mdate,version,cdate,classname,state,wid,fieldvalues,postitid,lockdomainid,domainid,cvid,name,dprofid,prelid,atags,confidential,ldapdn) values (NEW.id,NEW.owner,NEW.title,NEW.revision,NEW.initid,NEW.fromid,NEW.doctype,NEW.locked,NEW.allocated,NEW.icon,NEW.lmodify,NEW.profid,NEW.views,NEW.usefor,NEW.mdate,NEW.version,NEW.cdate,NEW.classname,NEW.state,NEW.wid,NEW.fieldvalues,NEW.postitid,NEW.lockdomainid,NEW.domainid,NEW.cvid,NEW.name,NEW.dprofid,NEW.prelid,NEW.atags,NEW.confidential,NEW.ldapdn);
     end if;
  end if;
end if;
