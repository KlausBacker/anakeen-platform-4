...
{
    global $action;
    $urlindex = \Anakeen\Core\ContextManager::getParameterValue("TE_URLINDEX");
    if ($urlindex == "") { //case DAV
        $au = \Anakeen\Core\ContextManager::getParameterValue("CORE_URLINDEX");
        if ($au != "") {
            $urlindex = \Anakeen\Core\ContextManager::getParameterValue("CORE_URLINDEX");
        } else {
            $scheme = \Anakeen\Core\ContextManager::getParameterValue("CORE_EXTERNURL");
            if ($scheme == "") {
...
function vault_generate($dbaccess, $engine, $vidin, $vidout, $isimage = false, $docid = '')
{
    $err = '';
    if (($vidin > 0) && ($vidout > 0)) {
        $tea = \Anakeen\Core\ContextManager::getParameterValue("TE_ACTIVATE");
        ...
        $ot = new \Dcp\TransformationEngine\Client(\Anakeen\Core\ContextManager::getParameterValue("TE_HOST"), \Anakeen\Core\ContextManager::getParameterValue("TE_PORT"));
...
function sendTextTransformation($dbaccess, $docid, $attrid, $index, $vid)
{
    ...
    if (($docid > 0) && ($vid > 0)) {
        ...
        $tea = \Anakeen\Core\ContextManager::getParameterValue("TE_FULLTEXT");
        if ($tea != "yes") {
            return '';
        }
