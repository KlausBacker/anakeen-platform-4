...
class DocLDAP extends DbObj
{
    ...
    public function OrgInit()
    {
        ...
        if (preg_match("/.*o=(.*),.*/", $this->racine, $reg)) {
            $orgldap["o"] = $reg[1];
        } // get organisation from LDAP_ROOT
        else {
            $orgldap["o"] = "unknown";
        }
     ...
     */
    public function RefreshLdapCard()
    {
        if (!$this->UseLdap()) {
            return false;
        }
     ...
     */
    public function DeleteLdapCard()
    {
        $err = '';
        if (!$this->UseLdap()) {
            return '';
     ...
     * get DN of document
     */
    public function getLDAPDN($rdn, $path = "")
    {
        if (!$rdn) {
            return false;
        }
        ...
        if ($path == "") {
            $dn = "$rdn=" . $vdn . "," . $this->racine;
        } else {
            $dn = "$rdn=" . $vdn . ",$path," . $this->racine;
        }
        return $dn;
    }
            ...
            if ($map) {
                if (substr($map, 0, 2) == "::") {
                    ...
                    if ($value) {
                        ...
                        if ((!isset($this->infoldap[$index]["objectClass"]))
                            || (!in_array($v["ldapclass"], $this->infoldap[$index]["objectClass"]))) {
                            $this->infoldap[$index]["objectClass"][] = $v["ldapclass"];
                        }
                    }
                } else {
                    switch ($map) {
     ...
     */
    public function getLDAPValue($idattr, $index = "")
    {
        ...
        } elseif (!isset($this->infoldap[$index])) {
            return null;
        } else {
            $tldap = $this->infoldap[$index];
        }
     ...
     */
    public function ModifyLdapCard($tinfoldap)
    {
        if (!$this->UseLdap()) {
            return false;
        }
                            ...
                            foreach ($tdn as $dn) {
                                ...
                                if ($sr) {
                                    ...
                                    for ($i = 0; $i < $attrs["count"]; $i++) {
                                        if (!isset($infoldap[$attrs[$i]])) {
                                            $delldap[$attrs[$i]] = array();
                                        }
                                    }
                                    if (count($delldap) > 0) {
     ...
     */
    public function createLDAPDc($ds, $n)
    {
        if ($ds) {
