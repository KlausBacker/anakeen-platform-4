...
function default_page(Action & $action)
{
    ...
    $action->parent->addCssRef(sprintf("%s:%s", $action->parent->name, "default_page.css"));
    ...
    $defaultValues = function ($value) use ($baseURL) {
        ...
        $value["acceptExtensions"] = isset($value["acceptExtensions"]) ? implode(", ", $value["acceptExtensions"])
            : "json";
        return $value;
    };
