...
function tengine_monitor_selftests(Action & $action)
{
    ...
    switch ($op) {
        case '':
            ...
            $action->parent->AddCssRef("TENGINE_MONITOR:tengine_monitor_selftests.min.css", true);
            $action->parent->AddJsRef("legacy/jquery-ui/js/jquery-ui.js");
            $action->parent->AddJsRef("TENGINE_MONITOR:tengine_monitor.js", true);
            $action->parent->AddJsRef("TENGINE_MONITOR:tengine_monitor_selftests.js", true);
...
function _tasks(Action & $action, $select)
{
    ...
    try {
        ...
        return sprintf(_("TE:Monitor:Malformed JSON argument: %s") , $e->getMessage());
    }
    $err = \Dcp\TransformationEngine\Manager::checkParameters();
    if ($err != '') {
        return $err;
...
function _abort(Action & $action, $tid)
{
    ...
    $te = new \Dcp\TransformationEngine\Client($action->getParam("TE_HOST") , $action->getParam("TE_PORT"));
    return $te->abortTransformation($tid);
}
