...
const {processJsonFile} = require(path.resolve(__dirname,'./util/json'));
const logger = require(path.resolve(__dirname,'./util/logger'));
const Workspace = require(path.resolve(__dirname,'./workspace'));
...
async function bumpVersion(depName, newVersion, dir) {
    ...
    const mismatchDepents = Object.keys(workspaceStart.packages).filter(key => {
        const deps = workspaceStart.packages[key]['mismatchedWorkspaceDependencies'];
        return deps.indexOf(depName) > -1;
    });
    ...
    for (secondName of mismatchDepents) {
        const secondPkg = workspaceStart.packageFilepath(secondName);
        processing.push(processJsonFile(
            secondPkg,
            json => replaceVersion(depName, newVersion, json)
