...
class CertificationTransitionsAccess implements IRenderTransitionAccess
{
    ...
    {
        return new CertificationTransitions();
    }
}


class CertificationTransitions extends \Dcp\Ui\TransitionRender
{
    /**
    ...
    public function getRenderOptions($transitionId)
    {
        ...
        $options->frame(self::parameterFrameAttribute)->setLabelPosition(\Dcp\Ui\CommonRenderOptions::nonePosition);
        $options->enum("wcert_checklist")->setDisplay(\Dcp\Ui\EnumRenderOptions::verticalDisplay);
        return $options;
    }
}