...
class FeesTransitions extends \Dcp\Ui\TransitionRender
{
    ...
    public function getRenderOptions($transitionId)
    {
        ...
        $options->commonOption(self::parameterFrameAttribute)->setLabelPosition(
    ...
    public function getTemplates($transitionId)
    {
        ...
        if ($transitionId === WFees::T_BA_EXCEED_DEMAND ||
            $transitionId === WFees::T_BA_SEND) {
            return $this->getUserTemplate($templates);
        } else if ($transitionId === WFees::T_BA_EXCEED_RESPONSE) {
            return $this->getDirectorTemplate($templates);
        }
        return $templates;
    }

    protected function getExceededCategoriesString($categories) {
        $message = "";
        if (!empty($categories)) {
            $message .= "Les montants forfaitaires pour les catégories suivantes ont été dépassés :<br><ul>";
    ...
    protected function getUserTemplate($templates) {
        ...
        $message = $this->getExceededCategoriesString($categories);
        if (!empty($categories)) {
            $message .= "<br>Souhaitez-vous <b>confirmer la demande de dépassement temporaire</b> ou <b>annuler l'envoi et modifier les dépenses ?</b>";

        } else {
            $message .= "En cliquant sur <b>Confirmer</b>, vous signez votre note de frais et l'envoyez en validation auprès de votre supérieur hiérarchique. <br>Souhaitez-vous continuer ?";
        }
        $templates["body"]= "{{>transitionHeader}}<h6>$message</h6>{{>transitionMessages}}{{>transitionButtons}}";
        return $templates;
    }

    protected function getDirectorTemplate($templates) {
        $doc = $this->getWorkflow()->doc;
        $outgoings = $doc->getArrayRawValues(FeesAttr::fee_t_all_exp);
        $categoriesRules = $this->getLimitsValues($doc);
        $exceeds = array_filter($outgoings, function ($o) {
        ...
        $message = $this->getExceededCategoriesString($categories);
        $templates["body"]= "{{>transitionHeader}}<h6>$message</h6>{{>transitionAsk}}{{>transitionMessages}}{{>transitionButtons}}";
        return $templates;
    }
}