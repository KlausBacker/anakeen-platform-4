...
function computePath(pathObj) {
    ...
    if (index > -1) {
        return (this.ignoreEmptyValue ? this.pathSeparator:'')
            + this.paths.slice(0, index + 1).map(p => p.id).join(this.pathSeparator);
    }
...
export default {
    ...
    methods: {
        onBreadcrumbItemClick(item) {
            this.$emit('itemClick', Object.assign({}, item, { path: computePath.call(this, item)}));
        },
        onRootClick() {
