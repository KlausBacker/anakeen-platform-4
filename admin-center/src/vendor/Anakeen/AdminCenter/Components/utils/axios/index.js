...
export default function install(Vue, options = {// jscs:ignore disallowFunctionDeclarations
    ...
    [axiosAdmin, axiosApiV2, axiosGlobal].forEach(axios => {
        axios.interceptors.request.use(options.onSuccessRequest, options.onErrorRequest);
        axios.interceptors.response.use(options.onSuccessResponse, options.onErrorResponse);
    });
