  ...
  return function initTest(type, widget, options, value, expected) {
    var currentSandbox,
      ...
      findWidgetName = function findWidgetName($element) {
        return _.find(_.keys($element.data()), function findfindWidgetName(
          currentKey
        ) {
    ...
    describe(type + " fileTest", function testFileTest() {
      ...
      if (options.mode === "read") {
        describe(
          type + " : setDownloadInline",
          function testsetDownloadInline() {
            it("setDownloadInline", function testsetDownloadInline() {
              var $sandBox = getSandbox(),
                attrValue;
              widget.call(
                $sandBox,
                _.defaults({ attributeValue: value }, options)
              );
              expect(
                $sandBox.find(".dcpAttribute__value a").attr("href")
              ).toContain(
                "&inline=" +
                  (options.renderOptions.contentDisposition ? "yes" : "no")
              );
              expect(
                $sandBox.find(".dcpAttribute__value a").attr("href")
              ).not.toContain(
                "&inline=" +
                  (options.renderOptions.contentDisposition ? "no" : "yes")
              );
            });
          }
        );
