    ...
    var init_grid = function require_grid(_, $) {
        ...
        var columnRender = {
            ...
            state: function state(cellData) {
                ...
                var currentState = cellData.properties.state || {}, html = '',
            ...
            title: function title(cellData) {
                var currentTitle = cellData.properties.title || "", html = '';
                if (this.withIcon) {
            ...
            openTitle: function openTitle(cellData) {
                var url = columnRender.prepareUrl(cellData), title = cellData.properties.title || "", html = '';
                if (this.withIcon) {
                    html = columnRender._icon(cellData.properties);
                }
                if (url) {
                    html += $('<a/>').addClass("openDoc overlay")
                      ...
                      .attr("data-revision", cellData.properties.revision).text(cellData.properties.title)[0].outerHTML;
            ...
            docid: function docid(cellData) {
                var attributes = cellData.attributes || [], value = attributes[this.id] || "", withIcon = this.withIcon;
                value = columnRender._multiplicatorAttributeAnalysis(value, function formatter(data) {
                    var html = "", revision = _.isNumber(data.revision) ? data.revision : -1, url;
                    if (withIcon) {
                    ...
                    if (data.value) {
                        ...
                        html += $('<a/>').addClass("openDoc overlay")
                          ...
                          .attr("data-revision", revision).text(data.displayValue)[0].outerHTML;
                    } else {
            ...
            color: function color(cellData) {
                var attributes = cellData.attributes || [], value = attributes[this.id] || "";
                value = columnRender._multiplicatorAttributeAnalysis(value, function formatter(data) {
                    if (data.value) {
        ...
        var serverData = function serverData(data, callback) {
            var url,
              ...
              addSeparator = function addSeparator(url) {
                  ...
                  return url;
              }, currentWidget = this, constructData = {},
        ...
        $.widget("dcp.docGrid", {
            ...
            _setOption: function _setOption(key, value) {
                if (key === "criterias") {
                    ...
                    this._error(["Unable to change option " + key +
            ...
            _initiateDocGrid: function _initiateDocGrid() {
                var element = this.element, trigger = $.bind(this._trigger, this);
            ...
            _initiateHeader: function _initiateHeader() {
                var $tr, tHead, HeaderElement = [], currentWidget = this,
                  analyseColumns = function analyseColumns(currentColumn) {
                      var currentHeaderCallback = currentColumn.HeaderCallBack || "", target = currentColumn.targets;
                      if (currentWidget.options.dataTableOptions.columnDefs !== undefined &&
                        currentWidget.options.dataTableOptions.columnDefs[target] !== undefined &&
                        currentWidget.options.dataTableOptions.columnDefs[target] !== null &&
                        currentWidget.options.dataTableOptions.columnDefs[target].visible === false) {
                          return;
                      }
                      HeaderElement[target] = _.isFunction(currentHeaderCallback) ?
                        currentHeaderCallback(currentColumn) : currentHeaderCallback;
                  },
                  htmlElement = "";
                $tr = $("<tr />");
            ...
            _initiateFooter: function _initiateFooter() {
                var $tr, HeaderElement = [], currentWidget = this, $tfoot,
                  analyseColumns = function analyseColumns(currentColumn) {
            ...
            _generateDataTableColumns: function _generateDataTableColumns() {
                var length, dataTableColumns = [], i, currentColumn;
            ...
            _productDataTableColumn: function _productDataTableColumn(value) {
                var type = value.type || '', column = {}, render;
            ...
            _HeaderFilterRender: {
                ...
                simple: function simple(value) {
                    ...
                    if (value.id && value.filterable) {
                        ...
                        if (kind === "enum") {
                            return '<div class="form-group"><div class="input-group"><input class="filter enumFilter combobox" ' +
                              'type="text" ' +
                              'data-id="' + id + '" data-famId="' + value.famId + '" data-type="' + kind + '"/>' +
                            ...
                            return '<div class="form-group"><div class="input-group"><input class="filter stateFilter combobox" ' +
                              'type="text" ' +
                              'data-id="' + id + '" data-famId="' + value.famId + '" data-type="' + kind + '"/>' +
                              '<div class="input-group-addon removeButton">' +
                              '<button class="btn btn btn-default"><span class="glyphicon glyphicon-remove"></span></button>' +
                            ...
                            return '<input class="filter textFilter k-textbox" ' +
                              'type="search" ' +
                              'data-id="' + id + '" data-famId="' + value.famId + '" data-type="' + kind + '"/>';
                        }
                    }
                          ...
                          transport: {
                              ...
                              parameterMap: function mapsParameters(data, type) {
                                  if (data.filter && data.filter.filters && data.filter.filters.length &&
                                    data.filter.filters[0] && _.isString(data.filter.filters[0].value)) {
