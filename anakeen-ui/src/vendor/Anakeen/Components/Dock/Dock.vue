    ...
    <div id="dock-component" :position="position">
        <div id="dock" :style="dockSizeStyle" :class="superposeDockClass">
            ...
            <div class="footer-tabs">
                <div :id="'footer.' + tab.id" :class="privateScope.selectedSelectable(tab)" class="footer-tab" :style="footerSizeStyle" v-for="tab in footerTabs" :key="tab.id" @click="selectTabWithId(tab.id)">
                    <span :id="'footer-compact.' + tab.id" class="footer-tab-compact" :style="'width: ' + compactSize" v-html="tab.compact"></span>
                    <span :id="'footer-expanded.' + tab.id" class="footer-tab-expanded" :style="expandedSizeStyle" v-html="tab.expanded" v-if="expandedDock && (position === 'left' || position === 'right')"></span>
                </div>
            </div>
        </div>
        <div id="content" :style="contentMarginStyle">
