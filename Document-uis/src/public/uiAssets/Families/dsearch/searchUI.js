...
require([
    ...
    'dcpContextRoot/uiAssets/Families/dsearch/searchAttributes'
], function ($, _, i18n, searchAttributes) {
      ...
      function prepareEvents() {
          ...
          $(this).documentController("addEventListener", "ready",
            ...
            function prepareSearchDocUI(event, document) {
                ...
                $documentController.documentController("addEventListener",
                  "beforeClose",
                  {
                ...
                searchAttributes(famid).then(function requestAttributesSReady(data) {
                    myAttributes = [];
                    ...
                    $.getJSON("api/v1/search_UI_HTML5/operators/", function requestOperatorsSReady(data) {
                        ...
                        $r.done(function () {
                            $.each(($documentController.documentController("getValues").se_attrids), function eachDocAttridsSReady($index, myAttribute) {
                            ...
                            $(".dcpAttribute__value[name=se_keys]").each(function eachKeysSReady(index) {
                                ...
                                if (myAttribute !== undefined && myOperator !== undefined) {
                                     ...
                                     */
                                    if (!itemEmpty(myOperator) && (myOperator.value === "=" || myOperator.value === "!=" || myOperator.value === "~y")) {
                                    ...
                                    if ($methods && $methods.length !== 0) {
                                        $($(".dcpAttribute__content[data-attrid=se_keys]")[index]).find("span:first").addClass("button--on").removeClass("button--off");
                                        $($(".dcpAttribute__content[data-attrid=se_keys]")[index]).find("input:first").addClass("button--on").removeClass("button--off");
                                        createButtonMethods($environment, $methods);
                                    }
                                    else {
                                        $($(".dcpAttribute__content[data-attrid=se_keys]")[index]).find("span:first").removeClass("button--on").addClass("button--off");
                                        $($(".dcpAttribute__content[data-attrid=se_keys]")[index]).find("input:first").removeClass("button--on").addClass("button--off");
                                        deleteButtonMethods($environment);
                                    }

                                }
                            });
                            $.each(($documentController.documentController("getValues").se_funcs), function eachDocFuncsSReady($index, myOperator) {
                ...
                if (!itemEmpty(myAttribute) && attributeExists) {
                    ...
                    if (myAttribute.value !== null) {
                        $.each(myAttributes, function eachPersoAttributesAttridsChanged(key, value) {
                            if (myAttribute.value === value.id) {
                                myChangedAttribute = {id: value.id, label: value.label, type: value.type};
                    ...
                    $environment.data("kendoDropDownList").select($environment.data("kendoDropDownList").ul.children().eq(0));
                    ...
                    if ($environment.closest("div").find("button").length !== 0) {
                        if ($checkMeth) {
                            $environment.closest("div").find("button").trigger("click");
                        }
                    }
                    deleteButtonMethods($environment);
                    ...
                    if ($funcEl !== undefined) {
                        if (!itemEmpty($funcEl) && ($funcEl.dataset.role !== undefined)) { // role is undefined if there is no widget
                        ...
                        if (!itemEmpty($environment) && ($environment.parent()[0].children.length === 3 || $environment.parent()[0].children.length === 4)) {
                            destroyDatePicker($environment);
                        }
                        if (!itemEmpty($environment) && ($environment.data("kendoComboBox") !== undefined) && ($environment[0].aNode !== undefined)) {
                            $environment.data("kendoComboBox").destroy();
                            $nodeToSave = $environment[0].aNode;
                            $parent = $environment.parent()[0].parentElement;
                            $parent.firstElementChild.remove();
          ...
          $(this).documentController("addEventListener", "change",
            ...
            function displayFuncsFamidChange() {
                ...
                searchAttributes(famid).then(function requestAttributesFamidChanged(data) {
                    myAttributes = [];
                    ...
                    testWorkflow = findIfWorkflow(myAttributes, $documentController);
                }).done(function doneFamidChanged() {
                    if (testWorkflow) {
          ...
          $(this).documentController("addEventListener", "change",
            ...
            function displayConditionChange(event, document, attribute, values) {
                ...
                else if (((values.current.value === "perso") && (values.previous.value === "or")) || (values.current.value === "or")) {
            ...
            function displayFuncChange(event, document, attribute, values) {
                ...
                if (!itemEmpty(myOperator)) {
                    if (myOperator.value !== "is null" && myOperator.value !== "is not null" && myOperator.value !== "><") {
                        ...
                        if ($type === "date") {
                            $environment.parent().find(".k-picker-wrap input").val("");
                        }
                ...
                if (myAttribute !== undefined && myOperator !== undefined) {
                    /*
                     if the widget is a comboBox, the current input is saved in the place where it should be
                     */
                    if (!itemEmpty(myOperator) && !($checkMeth) &&
                    ...
                    else if (!itemEmpty(myOperator) && ($type === "docid" || $type === "docid[]") &&
                    ...
                    if (!itemEmpty(myOperator) && ($type === "date" || $type === "timestamp" || $type === "time") && (myOperator.value !== "is null" && myOperator.value !== "is not null" && myOperator.value !== "><")) {
                        if ($environment.parent()[0].children.length === 1) {
                            var $input = $('<input />').attr({'type': 'text'}); // create a second input to separate value and display value
                            $input.insertBefore($environment[0]);
                            $environment.hide();
                 ...
                 */
                    ...
                    if (!$init && (!itemEmpty($environment)) && ($environment.parent()[0].children.length === 2 || $environment.parent()[0].children.length === 3)) {
                        destroyDatePicker($environment);
                    }
                    if (!$init && !itemEmpty($environment) && ($environment.data("kendoComboBox") !== undefined) && ($environment[0].aNode !== undefined)) {
                        $environment.data("kendoComboBox").destroy();
                        $nodeToSave = $environment[0].aNode;
                        $parent = $environment.parent()[0].parentElement;
                        $parent.firstElementChild.remove();
    ...
    function conditionVisibility($documentController) {
        ...
        if ($condition && $condition.value !== "perso") {
            $(".dcpArray__content[data-attrid=se_t_detail] > table").removeClass("dcpArray--custom").addClass("dcpArray--not_custom");
            ...
            $(".dcpArray__content[data-attrid=se_t_detail] > table").removeClass("dcpArray--not_custom").addClass("dcpArray--custom");
    ...
    function initDateTimePicker($environment, $index) {
        $environment.kendoDateTimePicker({
            ...
            change: function changeDatePickerValure() {
                ...
                var sTimeDate = "";
                if (timeDate) {
                    sTimeDate = timeDate.getFullYear() + '-' +
        ...
        $(".dcpAttribute__value[name=se_funcs]").each(function eachFuncsDefineDD(key) {
            if (key === $index) {
                ...
                if ($(this).data("kendoDropDownList") !== undefined) {
                    ...
                    $(this).data("kendoDropDownList").setDataSource($dataSource);
                }
                else {
                    ...
                    $(this).data("kendoDropDownList").list.css('min-width', '300px');
                }
                /*
            ...
            dataSource: {
                ...
                transport: {
                    ...
                    read: function readDatasIEnum(options) {
                        ...
                        $.ajax({
                            type: "GET",
                            url: "api/v1/families/" + famid + "/enumerates/" + attrid,
                            data: filter,
                            dataType: "json",
    ...
    function initOperators(data, myChangedAttribute, operators) {
        if (myChangedAttribute !== null && myChangedAttribute !== undefined) {
            $.each(data, function eachDataInitOperators(key, value) {
                 ...
                 */
                else if (value.compatibleTypes.indexOf(myChangedAttribute.type) !== -1) {
                    myObject = {
                        monId: value.id,
                        monTitre: value.title
                    };
    ...
    function createButtonMethods($environment, $methods) {
        ...
        $myButton.click(function clickOnMethodsButton() {
            if ($beforeMeth[$index] == null) {
                ...
                $checkMeth = true;
                $environment.data("kendoComboBox").list.css('min-width', '300px');
                $($environment.closest("div").find("span")).addClass("button--on").removeClass("button--off");
            }
            else {
                $environment.data("kendoComboBox").destroy();
