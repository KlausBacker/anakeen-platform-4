    ...
    public function testAccessApplication($data)
    {
        ...
        foreach ($data['tests'] as $testIdx => & $test) {
            ...
            foreach ($test['has:permission'] as $checkIdx => $check) {
                $user = SEManager::getDocument($check['user']);
                $this->assertTrue(is_object($user), sprintf("test#%s/check#%s> Could not get user with id '%s'.", $testIdx, $checkIdx, $check['user']));
                ...
                $perm = $myApp->hasPermission($check['acl'], $check['app']);
                $err = sprintf("test#%s/check#%s> Unexpected permission %s (should be %s) for user %s on acl %s from app %s", $testIdx, $checkIdx, $perm ? 'true' : 'false',
                    $check['permission'] ? 'true' : 'false', $check['user'], $check['acl'], $check['app']);
                if ($perm != $check['permission']) {
                    // these requests can be really slow, only execute them if needed
                    $err .= "\n\t" . $this->prettySqlRelation(sprintf("Groups test#%s/check#%s", $testIdx, $checkIdx),
                            "SELECT l.login AS user, r.login AS group FROM users AS l, groups AS g, users AS r WHERE g.iduser = l.id AND g.idgroup = r.id");
                    $err .= "\n\t" . $this->prettySqlRelation(sprintf("Permission test#%s/check#%s", $testIdx, $checkIdx),
                ...
                $configInfo = RouterLib::getRouteInfo($check['action']);

                try {
                    RouterAccess::checkRouteAccess($configInfo, true);
                    $perm = true;
                } catch (Exception $e) {
                    $perm = $e->getMessage();
                }
                ...
                if ($check['permission']) {
                    $this->assertTrue(
                        true === $perm,
                        sprintf(
                            "test#%s/check#%s> Unexpected error %s (should be true) for user %s on action %s from app %s",
                            $testIdx,
                            $checkIdx,
                            var_export($perm, true),
                            $check['user'],
                            $check['action'],
                            $check['app']
                        )
                    ...
                    $regexp = sprintf('/ROUTES0131.* "%s"/', $check['action']);
                    $this->assertRegExp(
                        $regexp,
                        $perm,
                        sprintf(
                            "test#%s/check#%s> Unexpected error %s (should match %s) for user %s on action %s from app %s",
                            $testIdx,
                            $checkIdx,
                            var_export($perm, true),
                            $regexp,
