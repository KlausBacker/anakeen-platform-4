...
class Module
{
    ...
    public function download($status = '')
    {
        ...
        if ($this->tmpfile === false) {
            $this->errorMessage = sprintf("Could not download '%s': %s", WIFF::anonymizeUrl($modUrl),
                $wiff->errorMessage);
            return false;
        }
        ...
        if ($modulesNodeList->length <= 0) {
            $this->errorMessage = sprintf("Found no <modules> for context '%s' in '%s'.", $this->context->name,
                $wiff->contexts_filepath);
            return false;
        }
        $modulesNode = $modulesNodeList->item(0);
        // Look for an existing <module> node
        if ($status == 'downloaded') {
            $query = sprintf("/contexts/context[@name='%s']/modules/module[@name='%s' and @status='downloaded']",
                $this->context->name, $this->name);
        } else {
            $query = sprintf("/contexts/context[@name='%s']/modules/module[@name='%s']", $this->context->name,
                $this->name);
        }
        ...
        if ($existingModuleNodeList->length <= 0) {
            ...
            if ($existingModuleNodeList->length > 1) {
                $this->errorMessage = sprintf("Found more than one <module> with name='%s' in '%s'.", $this->name,
                    $wiff->contexts_filepath);
                return false;
            }
                ...
                if ($module->name != $this->name) {
                    ...
                    foreach ($installedManifest as $manifest) {
                        if ($manifest["type"] != "d" && in_array($manifest["name"], $modulesManifestFiles)) {
                            $this->errorMessage = sprintf("File '%s' is already given by module '%s'. If you skip this, this file will be replaced by the new one.",
    ...
    {
        if (!is_file($this->tmpfile)) {
            $this->errorMessage = sprintf("Temporary file \"%s\" of downloaded module does not exists.",
                $this->tmpfile);
            return false;
        }
        // Store BOM/manifest
        ...
        if ($ret === false) {
            $this->errorMessage = sprintf("Error getting manifest for '%s': %s", $this->name,
    ...
    function fGetDisplayableParameterList()
    {
        ...
        foreach ($plist as $p) {
            if ($p->onedit == '' || $p->onedit == 'W' || $p->onedit == 'R') {
                return true;
            }
        }
        return false;
    ...
    public function storeParameter(Parameter $parameter)
    {
        ...
        $parametersValueList = $contextsXpath->query(sprintf("/contexts/context[@name='%s']/parameters-value",
            $this->context->name));
        ...
        $paramList = $contextsXpath->query(sprintf("/contexts/context[@name='%s']/parameters-value/param[@modulename='%s' and @name='%s']",
            $this->context->name, $this->name, $parameter->name));
        ...
        $ret = $wiff->commitDOMDocument($xml);
        if ($ret === false) {
            $this->errorMessage = sprintf("Error saving XML to '%s': %s", $wiff->contexts_filepath,
                $wiff->errorMessage);
            return false;
        }
    ...
    public function setStatus($status, $errorstatus = null)
    {
        ...
        $xpath = new DOMXpath($xml);
        $moduleNodeList = $xpath->query(sprintf("/contexts/context[@name='%s']/modules/module[@name='%s']",
            $this->context->name, $this->name));
        if ($moduleNodeList->length <= 0) {
            $this->errorMessage = sprintf("Could not find module '%s' in context '%s'!", $this->name,
                $this->context->name);
            return false;
        }
        /**
